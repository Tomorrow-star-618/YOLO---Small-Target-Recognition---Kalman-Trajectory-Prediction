...
class GradientMagnitudeAnalyzer:
    ...
    def _create_score_distribution_plot(self, result, image_name, output_dir):
        """创建评分分布图"""
        scores = [w['score'] for w in result['all_windows']]
        ...
        # 评分vs位置散点图
        x_coords = [w['center'][0] for w in result['all_windows']]
        y_coords = [w['center'][1] for w in result['all_windows']]
        scatter = ax2.scatter(x_coords, y_coords, c=scores, cmap='hot', alpha=0.6, s=10)
        ax2.plot(result['best_center'][0], result['best_center'][1], 
                'r*', markersize=15, label='Best Position')
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f'{image_name}_score_distribution.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _create_top_windows_comparison(self, result, image_name, output_dir):
        """创建top窗口对比图"""
        # 按评分排序，取前6个
        top_windows = sorted(result['all_windows'], key=lambda x: x['score'], reverse=True)[:6]
        
        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        axes = axes.flatten()
        
        for i, window_info in enumerate(top_windows):
            ax = axes[i]
            window = window_info['window']
            
            ax.imshow(window, cmap='gray')
            ax.set_title(f'Rank {i+1}: Score {window_info["score"]:.3f}\n'
                        f'Pos: {window_info["center"]}\n'
                        f'Mean: {window_info["window_mean"]:.1f}, '
                        f'Grad: {window_info["gradient_mean"]:.1f}', 
                        fontsize=10)
            ax.plot(self.half_size, self.half_size, 'r+', markersize=8, markeredgewidth=2)
            
            # 添加数值标注
            for row in range(self.roi_size):
                for col in range(self.roi_size):
                    ax.text(col, row, f'{window[row, col]}', 
                           ha='center', va='center', fontsize=7, color='yellow')
        
        plt.tight_layout()
        plt.savefig(output_dir / f'{image_name}_top_windows.png', dpi=300, bbox_inches='tight')
        plt.close()
    
    def _save_analysis_report(self, result, image_path, output_dir, enable_circular=False):
        """保存详细分析报告"""
        image_name = Path(image_path).stem
        report_path = output_dir / f'{image_name}_gradient_analysis_report.txt'
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            if enable_circular:
                ...
                if result['gradient_best_window']:
                    ...
                    f.write(f"    Score = {best.get('window_mean', 0):.3f} + 0.3 × {best.get('gradient_mean', 0):.3f}\n")
                    f.write(f"    Score = {result['gradient_best_score']:.6f}\n")
                f.write("\n")
                ...
                f.write("  📊 评分公式:\n")
                f.write("    Score = 加权平均强度 + 0.2 × 中心对比度\n")
                f.write("\n")
                ...
                if result['gradient_best_center'] == result['circular_best_center']:
                    ...
                    f.write(f"    梯度法: {result['gradient_best_center']}\n")
                    f.write(f"    圆形法: {result['circular_best_center']}\n")
                ...
                # 最佳窗口展示（以得分更高的为准）
                if grad_score >= circ_score and result['gradient_best_window']:
                    f.write("🎯 最佳窗口灰度矩阵（梯度法）:\n")
                    window = result['gradient_best_window']['window']
                elif result['circular_best_window']:
                    f.write("🎯 最佳窗口灰度矩阵（圆形法）:\n")
                    window = result['circular_best_window']['window']
                else:
                    window = None
                ...
                # Top 5 梯度法结果
                top_5_grad = sorted(result['gradient_results'], key=lambda x: x['score'], reverse=True)[:5]
                f.write("🔝 梯度法 Top 5 候选位置:\n")
                f.write("  排名   中心位置    评分\n")
                f.write("  " + "-" * 30 + "\n")
                for i, window in enumerate(top_5_grad):
                    f.write(f"  {i+1:2d}    {window['center']}   {window['score']:8.3f}\n")
                f.write("\n")
                
                # Top 5 圆形法结果
                top_5_circ = sorted(result['circular_results'], key=lambda x: x['score'], reverse=True)[:5]
                f.write("🔝 圆形法 Top 5 候选位置:\n")
                f.write("  排名   中心位置    评分\n")
                f.write("  " + "-" * 30 + "\n")
                for i, window in enumerate(top_5_circ):
                    f.write(f"  {i+1:2d}    {window['center']}   {window['score']:8.3f}\n")
