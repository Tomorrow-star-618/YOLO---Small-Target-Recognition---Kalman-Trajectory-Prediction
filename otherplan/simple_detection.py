...
class SimpleDetector:
    ...
    def process_frame(self, frame_bgr, prev_cx=None):
        ...
        if not SKIP_VISUALIZATION and target_count > 0:
            for i, det in enumerate(detections):
                ...
                cv2.putText(vis, label, (det.x1, max(0, det.y1 - 6)), FONT, 0.5, color, 1, cv2.LINE_AA)
        ...
        if not SKIP_VISUALIZATION:
            cv2.putText(vis, f"F:{self.frame_idx}", (10, 24), FONT, 0.6, (255, 255, 255), 1)  # 简化文字
            if target_count > 0:
                cv2.putText(vis, f"T:{target_count}", (10, 45), FONT, 0.6, (255, 255, 255), 1)
            # 显示已保存的多目标图像数量
            if SAVE_MULTI_TARGET_IMAGES and self.saved_images_count > 0:
                cv2.putText(vis, f"Saved:{self.saved_images_count}", (10, 66), FONT, 0.6, (0, 255, 255), 1)
    ...
    def write_csv_row_fast(self, detections):
        ...
        if len(detections) > 0:
            # 只记录第一个（最佳）目标
            det = detections[0]
            row = [self.frame_idx, f"{t:.2f}", det.source, f"{det.conf:.2f}", int(det.cx), int(det.cy), len(detections)]
    ...
    def write_csv_row(self, detections):
        ...
        if len(detections) > 0:
            ...
            row = [self.frame_idx, f"{t:.3f}", "none", "none", "0.000", -1, -1, -1, -1, -1, -1, 0]
            self.csv_writer.writerow(row)
    ...
    def print_progress(self, force_print=False):
        ...
        print(
            f"[PROGRESS] 帧: {self.frame_idx + 1:>6}/{self.total_frames} "
            f"({progress:>5.1f}%) | "
            f"处理速度: {fps:>5.1f} fps | "
            f"用时: {elapsed:>6.1f}s | "
            f"预计剩余: {eta:>6.1f}s | "
        ...
        if force_print:
            print(
                f"[STATS] 总检测: {self.total_detections}, "
            ...
            if fps < 20:
                print("[TIPS] 性能优化建议:")
                print("  - 降低输入视频分辨率")
                print("  - 设置 YOLO_IMG_SIZE = 320")
                print("  - 设置 FAST_MODE = True")
                print("  - 检查GPU可用性")
                if self.model is not None:
...
def parse_arguments():
    ...
    parser.add_argument("--enable-csv", action="store_true", help="启用CSV结果输出功能")
    parser.add_argument("--input", "-i", type=str, default=INPUT_VIDEO, help=f"输入视频路径 (默认: {INPUT_VIDEO})")
    parser.add_argument(
        "--weights", "-w", type=str, default=YOLO_WEIGHTS, help=f"YOLO权重文件路径 (默认: {YOLO_WEIGHTS})"
    )
    parser.add_argument("--fast-mode", action="store_true", help="启用快速处理模式")
    parser.add_argument("--enable-image-save", action="store_true", help="启用多目标图像保存功能")
...
def main():
    ...
    # 设置输出路径
    output_video, output_csv, save_images_dir = setup_output_paths(args.input, ENABLE_CSV_OUTPUT)
    ...
    if not writer.isOpened():
        ...
        for codec in ["XVID", "MJPG", "MP4V"]:
            ...
            print("[ERROR] 所有编码格式都失败，无法创建视频输出")
            return
