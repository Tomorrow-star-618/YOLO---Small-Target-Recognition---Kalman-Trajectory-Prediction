...
def create_output_path(input_video_path):
    """根据输入视频创建输出路径 格式：otherplan/runs/视频名-YYYYMMDD-HHMMSS/output-video/."""
    # 获取视频文件名（不含扩展名）
...
def classical_detect(gray: np.ndarray, frame_shape) -> list:
    """传统回退检测：返回候选框列表[(x1,y1,x2,y2,score), ...]。 仅在无YOLO或权重缺失时启用。."""
    H, W = frame_shape[:2]
...
class IRSmallTargetDetector:
        ...
        if self.model is not None:
            try:
                ...
                if self.frame_idx == 0 and yolo_raw_count == 0:
                    print("[WARN] YOLO未检测到任何目标，可能需要专用的红外小目标检测模型")
    ...
    def process_frame(self, frame_bgr):
        ...
        cv2.putText(
            vis,
            f"Frame: {self.frame_idx} | Detections: {len(detections)}",
            (10, 24),
            FONT,
            0.7,
            (255, 255, 255),
