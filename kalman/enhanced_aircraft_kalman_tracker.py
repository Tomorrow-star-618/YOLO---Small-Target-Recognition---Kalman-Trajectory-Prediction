...
class AircraftKalmanTracker:
    ...
    def analyze_motion_pattern(self):
        ...
        self.motion_analysis["prediction_confidence"] = self.motion_analysis["stability_score"] * data_confidence
    ...
    def _calculate_direction_consistency(self):
        ...
        direction_changes = np.array(
            [change if abs(change) < np.pi else change - 2 * np.pi * np.sign(change) for change in direction_changes]
        )
    ...
    def mark_as_lost(self):
        ...
        if not self.is_lost:
            ...
            print(
                f"目标 {self.track_id} 丢失 - 位置: [{pos[0]:.1f}, {pos[1]:.1f}], "
