...
class TrajectoryVisualizer:
    ...
    def _draw_detections(self, image, detections):
        ...
        for det in detections:
            if len(det) >= 5:
                ...
                cv2.putText(
                    image, f"Det: {conf:.2f}", (x1, y1 - 5), self.font, 0.3, self.colors["detected"], 1
                )  # 更小字体
    ...
    def _draw_single_track(self, image, track):
        ...
        cv2.putText(
            image, conf_text, (x2 + 10, y2 + 10), self.font, 0.3, self.colors["text"], 1
        )  # 更小字体，移到右下角
    ...
    def _draw_label(self, image, label, x1, y1, x2, y2, color):
        ...
        cv2.putText(
            image, label, (label_x, label_y), self.font, self.font_scale, self.colors["text"], self.font_thickness
        )
    ...
    def _draw_status_text(self, image, text, x2, y1, color):
        ...
        # 背景
        cv2.rectangle(
            image, (text_x - 2, text_y - text_size[1] - 2), (text_x + text_size[0] + 2, text_y + 2), color, -1
        )
        cv2.putText(image, text, (text_x, text_y), self.font, status_font_scale, (255, 255, 255), status_font_thickness)
    ...
    def _draw_trajectory(self, image, trajectory, color):
        ...
        for i in range(1, len(points)):
            ...
            cv2.line(image, tuple(points[i - 1]), tuple(points[i]), self.colors["trajectory"], thickness)
    ...
    def _draw_velocity_vector(self, image, bbox, velocity):
        ...
        cv2.arrowedLine(image, (center_x, center_y), (end_x, end_y), self.colors["velocity"], 2, tipLength=0.3)
    ...
    def _draw_frame_info(self, image, frame_info, tracks, detections):
        ...
        for i, text in enumerate(info_texts):
            ...
            cv2.putText(image, text, (10, y_pos), self.font, 0.6, self.colors["text"], 2)
    ...
    def _draw_legend(self, image):
        ...
        cv2.rectangle(image, (legend_x - 10, legend_y - 10), (w - 10, h - 10), self.colors["background"], -1)
        cv2.rectangle(image, (legend_x - 10, legend_y - 10), (w - 10, h - 10), self.colors["text"], 2)

        cv2.putText(image, "Status Legend", (legend_x, legend_y - 5), self.font, 0.6, self.colors["text"], 2)
        ...
        for i, (label, color) in enumerate(legends):
            ...
            cv2.putText(image, label, (legend_x + 25, y + 12), self.font, 0.45, self.colors["text"], 1)
